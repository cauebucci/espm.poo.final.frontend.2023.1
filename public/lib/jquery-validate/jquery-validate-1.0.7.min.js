...
(function () {
	...
	$.validator.addMethod("datahora", function (value, element) {
		if (!(value = trim(value)))
			return true;
		if (value.length =! 16)
			return false;
			
		const stamp = value.split("T");
		const data = stamp[0];
		const horario = stamp[1];

		if(!expHorario.test(horario)) 
			return false;
		...
		if (b1 <= 0 || b2 <= b1)
			return false;
		const ano = parseInt(data.substring(0, b1));
		const mes = parseInt(data.substring(b1 + 1, b2));
		const dia = parseInt(data.substring(b2 + 1));
		if (isNaN(dia) || isNaN(mes) || isNaN(ano) ||
			dia < 1 || mes < 1 || ano < 1 ||
			dia > 31 || mes > 12 || ano > 9999)
			return false;
